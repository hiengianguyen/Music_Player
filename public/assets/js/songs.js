const listSong = [
    "K_Right24", 
    "Anh Tu Voi", 
    "ATSH", 
    "RAPVIET4"
]

const songs = {
    "K_Right24": [
        {
          name: "Truy Lùng Báo Vật",
          singer: "24K.RIGHT ft Sofia, producer: Masew,...",
          image: "./assets/img/24kR.jpg",
          path: "./assets/music/truylungbaovat.mp3",
        },
        {
          name: "Khoá Chân",
          singer: "24K.RIGHT [Feat. MASON NGUYỄN, NAM COCAIN, TUANN]",
          image: "./assets/img/24kR.jpg",
          path: "./assets/music/khoachan.mp3",
        },
        {
          name: "Phụ Hộ Cho Con",
          singer: "24K.RIGHT [feat. B RAY, HUỲNH CÔNG HIẾU, HIPZ]",
          image: "./assets/img/24kR.jpg",
          path: "./assets/music/phuhochocon.mp3",
        },
        {
          name: "Bao Giờ Lấy Vợ",
          singer: "24K.RIGHT [feat. B RAY, HUỲNH CÔNG HIẾU, HIPZ]",
          image: "./assets/img/24kR.jpg",
          path: "./assets/music/phuhochocon.mp3",
        },
        {
          name: "Từ A đến Z",
          singer: "24K.RIGHT [feat. B RAY, HUỲNH CÔNG HIẾU, HIPZ]",
          image: "./assets/img/24kR.jpg",
          path: "./assets/music/phuhochocon.mp3",
        },
        {
          name: "Một Nửa Sự Thật",
          singer: "24K.RIGHT [feat. B RAY, HUỲNH CÔNG HIẾU, HIPZ]",
          image: "./assets/img/24kR.jpg",
          path: "./assets/music/phuhochocon.mp3",
        }
    ]
   , 
    "Anh Tu Voi": [
        {
            name: "Ngày Mai Người Ta Lấy Chồng",
            singer: "Vôi Bản Đôn",
            image: "./assets/img/anhtu.jpg",
            path: "./assets/music/ngaymainguoitalaychong.mp3",
        },
        {
            name: "Đừng Quên Tên Anh",
            singer: "Anh Tú",
            image: "./assets/img/anhtu.jpg",
            path: "./assets/music/dungquentenanh.mp3",
        },
        {
            name: "Duyên Do Trời, Phận Tại Ta",
            singer: "Anh Tú",
            image: "./assets/img/anhtu.jpg",
            path: "./assets/music/duyendotroiphantaita.mp3",
        },
        {
            name: "Rời Bỏ",
            singer: "Anh Tú",
            image: "./assets/img/anhtu.jpg",
            path: "./assets/music/roibo.mp3",
        },
        {
            name: "Đoá Hồng Chơi Vơi",
            singer: "Anh Tú",
            image: "./assets/img/anhtu.jpg",
            path: "./assets/music/doahongchoivoi.mp3",
        },
        {
            name: "Khoá Ly Biệt",
            singer: "Anh Tú",
            image: "./assets/img/anhtu.jpg",
            path: "./assets/music/khoalibiet.mp3",
        }
    ]
    ,
    "ATSH": [
        {
          name: "Love Sand",
          singer: "HTH,JSOL,ALI Hoàng Dương",
          image: "./assets/img/anhtraisayhi.jpg",
          path: "./assets/music/lovesand.mp3",
        },
        {
          name: "Reget",
          singer: "Quân A.P, Quang Trung, ALi HD, Pháp Kiều FT Lâm Bảo Ngọc",
          image: "./assets/img/anhtraisayhi.jpg",
          path: "./assets/music/reget.mp3",
        },
        {
            name: "Sao Hạng A",
            singer: "Song Luân x HIEUTHUHAI x JSOL x Dương Domic",
            image: "./assets/img/anhtraisayhi.jpg",
            path: "./assets/music/saohanga.mp3",
        },
        {
            name: "Đầu Đội Sừng",
            singer: "HURRYKNG x Quân AP x Gemini Hùng Huỳnh",
            image: "./assets/img/anhtraisayhi.jpg",
            path: "./assets/music/daudoisung.mp3",
        },
        {
            name: "Đều Là Của Em",
            singer: "Anh Tú x Atus x Song Luân x Dương Domic x Quang Trung",
            image: "./assets/img/anhtraisayhi.jpg",
            path: "./assets/music/deulacuaem.mp3",
        },
        {
            name: "Kim Phút Kim Giờ",
            singer: "Negav, Pháp Kiều, ISAAC, HIEUTHUHAI, HURRYKNG",
            image: "./assets/img/anhtraisayhi.jpg",
            path: "./assets/music/kimphutkimgio.mp3",
        },
        {
            name: "Trói Em Lại",
            singer: "Quang Hùng MasterD",
            image: "./assets/img/anhtraisayhi.jpg",
            path: "./assets/music/troiemlai.mp3",
        },
        {
            name: "Tinh Đầu Quá Chén",
            singer: "Quang Hùng MasterD, Negav, Erik, Pháp Kiều",
            image: "./assets/img/anhtraisayhi.jpg",
            path: "./assets/music/tinhdauquachen.mp3",
        },
        {
            name: "Quay Đi Quay Lại",
            singer: "HIEUTHUHAI",
            image: "./assets/img/anhtraisayhi.jpg",
            path: "./assets/music/quaydiquaylai.mp3",
        },
        {
            name: "Anh Biết Rồi",
            singer: "RHYDER",
            image: "./assets/img/anhtraisayhi.jpg",
            path: "./assets/music/anhbietroi.mp3",
        }
    ]
    ,
    "RAPVIET4": [
        {
            name: "Anh Đã Làm Gì Đâu",
            singer: "Nhật Hoàng",
            image: "./assets/img/rapviet.webp",
            path: "./assets/music/anhdalamgidau.mp3",
        },
        {
            name: "Quay Đi Quay Lại",
            singer: "HS Robber",
            image: "./assets/img/rapviet.webp",
            path: "./assets/music/1tinhyeu.mp3",
        },
        {
            name: "Quay Đi Quay Lại",
            singer: "HS Robber x Ngắn",
            image: "./assets/img/rapviet.webp",
            path: "./assets/music/quatungkhunghinh.mp3",
        },
        {
            name: "Quay Đi Quay Lại",
            singer: "HS Robber",
            image: "./assets/img/rapviet.webp",
            path: "./assets/music/hustlangallday.mp3",
        },
        {
            name: "Quay Đi Quay Lại",
            singer: "CoolKid x RHYDER",
            image: "./assets/img/rapviet.webp",
            path: "./assets/music/lailadghouse.mp3",
        },
        {
            name: "Quay Đi Quay Lại",
            singer: "Gill x Captain",
            image: "./assets/img/rapviet.webp",
            path: "./assets/music/chomotnguoi.mp3",
        },
        {
            name: "Quay Đi Quay Lại",
            singer: "DangRangTo",
            image: "./assets/img/rapviet.webp",
            path: "./assets/music/luottrenconsong.mp3",
        },
        {
            name: "Quay Đi Quay Lại",
            singer: "Gill",
            image: "./assets/img/rapviet.webp",
            path: "./assets/music/tuyettrenpho.mp3",
        },
        {
            name: "Quay Đi Quay Lại",
            singer: "CoolKid x YoungPuppy",
            image: "./assets/img/rapviet.webp",
            path: "./assets/music/lovesomebody.mp3",
        }
    ]
}

export {songs, listSong};